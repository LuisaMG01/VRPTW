...
OUTPUT_FILE = 'VRPTW_LuisaMariaAlvarez_multistart_els_tabu.xlsx'
...
class VRPTW:
    ...
    def construct_solution_multistart(self, num_starts=3):
        best_vehicles = None
        best_distance = float('inf')
...
def solve_instance(instance_path):
    ...
    solution = vrptw.construct_solution_multistart(num_starts=3)  # Cambia a multistart
    end_time = time.time()
    ...
    for instance_filename in os.listdir(INSTANCES_DIR):
        if instance_filename.endswith('.txt'):
            ...
            for i, (route, arrival_time, load) in enumerate(routes, 1):
                ...
                # Guardar resultados solo si se encontró la instancia
