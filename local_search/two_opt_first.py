...
OUTPUT_FILE = 'VRPTW_LuisaMariaAlvarez_two_opt_first_improvement.xlsx'
...
class VRPTW:
    ...
    def two_opt_first_improvement(self):
        improved = True
        while improved:
            improved = False
            for vehicle in self.vehicles:
                ...
                for i in range(1, len(route) - 2):
                    for j in range(i + 1, len(route) - 1):
                        if self.two_opt_swap(vehicle, i, j):
                            improved = True
                            break
                    if improved:
                        break
    ...
    def two_opt_swap(self, vehicle, i, j):
        new_route = vehicle.route[:i] + vehicle.route[i:j+1][::-1] + vehicle.route[j+1:]
        
        if self.is_route_feasible(new_route):
            old_distance = self.route_distance(vehicle.route)
...
def solve_instance(instance_path):
    ...
    vrptw.two_opt_first_improvement()
    end_time = time.time()
