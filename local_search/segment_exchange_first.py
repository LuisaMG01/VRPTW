...
OUTPUT_FILE = 'VRPTW_LuisaMariaAlvarez_segment_exchange_first_improvement.xlsx'
...
class VRPTW:
    ...
    def segment_exchange_first_improvement(self):
        improved = True
        while improved:
            improved = False
            for i in range(len(self.vehicles)):
                for j in range(i + 1, len(self.vehicles)):
                    if self.exchange_segments(self.vehicles[i], self.vehicles[j]):
                        improved = True
                        break
                if improved:
                    break

    def exchange_segments(self, vehicle1, vehicle2):
        for i in range(1, len(vehicle1.route) - 2):
            for j in range(1, len(vehicle2.route) - 2):
                for segment_length in range(1, min(len(vehicle1.route) - i - 1, len(vehicle2.route) - j - 1)):
                    ...
                    if (self.is_route_feasible(new_route1) and self.is_route_feasible(new_route2) and
                        self.route_distance(new_route1) + self.route_distance(new_route2) <
                        self.route_distance(vehicle1.route) + self.route_distance(vehicle2.route)):
...
def solve_instance(instance_path):
    ...
    vrptw.segment_exchange_first_improvement()
    end_time = time.time()
